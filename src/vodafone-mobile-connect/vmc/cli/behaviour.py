...
from vmc.common.behaviour import Behaviour
from vmc.cli.collaborator import CLICollaboratorFactory
from vmc.contrib.epsilon.modal import mode

class CLIBehaviour(Behaviour):
    Init = Behaviour.Init
    NetReg = Behaviour.NetReg
    ImDone = Behaviour.ImDone
    
    collaborator = CLICollaboratorFactory
    
    def __init__(self, device, dialer, sm_callbacks, sm_errbacks, conf):
        self.conf = conf
        super(CLIBehaviour, self).__init__(device, dialer,
                                           sm_callbacks, sm_errbacks)
    
    class Auth(mode):
        """
        We override Behaviour.Auth because we want a reference of the config
        """
        def __enter__(self):
            pass
        def __exit__(self):
        ...
        def do_next(self):
            ...
            authsm = authklass(self.device, self.collaborator, self.conf)
            d = authsm.start_auth()
