...
class SIMProtocol(BufferingStateMachine):
    ...
    L{queue_at_cmd} which accepts and queues an L{vmc.common.command.ATCmd}
    ...
    several redesigns and simplifications, this FSM has just two states:
     ...
     change states. This regexps are defined in L{vmc.common.command.CMD_DICT}
     although the plugin mechanism offers the possibility of customizing the
     CMD_DICT through L{vmc.common.hardware.Customizer} if a card uses a
