...
class Contact(object):
    ...
    def __init__(self, name, number, index=None, group='SIM'):
        super(Contact, self).__init__()
        self.name = to_u(name)
        self.number = to_u(number)
        self.index = index
        self.group = to_u(group)
    
    def __repr__(self):
        if self.index:
            args = (self.index, self.group, self.name, self.number)
            return '<Contact index="%d" group="%s" name="%s" number="%s">' % args
        
        return '<Contact group="%s" name="%s" number="%s">' % (self.group, self.name, self.number)
...
class DBContact(item.Item):
    ...
    def __repr__(self):
        return '<DBContact group="%s" name="%s" number="%s">' % (self.group, self.name, self.number)
...
class ContactsManager(AxiomDBManager):
    ...
    def add_contact(self, contact):
        return DBContact(store=self.store, name=contact.get_name(),
                        number=contact.get_number(), group=contact.get_group())
...
class UsageManager(AxiomDBManager):
    ...
    def add_usage_entry(self, umts, start, end, bits_recv, bits_sent):
        from os.path import join
        filename = start.strftime('%B').capitalize() \
                + '_' + start.strftime('%Y') + '.txt'
        fd = open (join (consts.VMC_HOME, filename), 'a')
        line = []
        line.append (start.strftime('%d/%m/%Y  %H:%M:%S')) # Start time
        line.append (end.strftime('%d/%m/%Y  %H:%M:%S')) # End time
        delta = end - start
        line.append (str(delta.days * 1440 + delta.seconds / 60)) # Minutes
        line.append (str(bits_sent / 8)) # Sent bytes
        line.append (str(bits_recv / 8)) # Recv bytes
        line.append (str((bits_sent + bits_recv) / 8)) # Total bytes
        # TODO: N -> National, R -> Roaming
        line.append ('N')
    ...
    def add_usage_item(self, umts, start, end, bits_recv, bits_sent):
        self.add_usage_entry(umts, start, end, bits_recv, bits_sent)
        return UsageItem(store=self.store,
                         umts=umts,
                         start_time=Time.fromDatetime(start),
                         end_time=Time.fromDatetime(end),
                         bits_recv=bits_recv,
                         bits_sent=bits_sent)
